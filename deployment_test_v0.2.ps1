...
$WindowsVersion = "Win10_1903_V1_English_x64"

# Mount the iso image
Mount-DiskImage -Imagepath C:\Deployment_Test\ISO\$($WindowsVersion).iso

# Find the install.esd file and copy over to the local drive
copy-item -Path D:\Sources\install.wim -Destination C:\Deployment_Test\WIM\$($Date)_$($WindowsVersion)_install.wim

# Un-mount the iso image
Dismount-DiskImage -Imagepath C:\Deployment_Test\ISO\$($WindowsVersion).iso
